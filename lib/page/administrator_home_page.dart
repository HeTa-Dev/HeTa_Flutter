...
class _AdministratorHomePage extends State<AdministratorHomePage> {
  ...
  Future<void> _getPostView() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        postViewList = jsonData.map((item) => PostView.fromJson(item)).toList();
        List<PostView>? tempPostViewList = [];
        for(var postView in postViewList!){
          if(postView.isReported == true){
            tempPostViewList.add(postView);
          }
        }
        postViewList = tempPostViewList;
      });
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    precacheImage(
        NetworkImage(
            "https://heta-images.oss-cn-shanghai.aliyuncs.com/1686646960611188.webp"),
        context);
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    final user = userProvider.user;
    precacheImage(
        NetworkImage(user?.avatarPath ?? WebConfig.DEFAULT_IMAGE_PATH),
        context);
